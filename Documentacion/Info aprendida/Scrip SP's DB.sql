...
CALL spInsertMateria(8, 'Matematica2', 'C:/Materias/Matematicas2/programa.pdf');
...
DELIMITER //
CREATE PROCEDURE spLogin(IN pi_dni INT, IN pi_clave VARCHAR(50), OUT po_idrol INT, OUT msj VARCHAR(250))
BEGIN
	DECLARE Cant int;  
...
set @po_idrol = 0;
set @msj = '';
call instituto_smdl.spLogin(1, '12345', @po_idrol, @msj);
select @po_idrol, @msj;

           SELECT *
             FROM USUARIO
             WHERE dni = 1 and clave = '12345';
             

  SELECT IF( EXISTS(SELECT * FROM USUARIO
					WHERE dni = pi_dni and clave = pi_clave), 1, 0) as Cant;
...
DELIMITER //
CREATE PROCEDURE spincrementarAlumnoCurso  (IN id INT)
BEGIN	
	UPDATE curso
    SET cantalum = cantalum+1        
    where idcurso = id; 
END //
DELIMITER ;

DELIMITER //
CREATE PROCEDURE spdecrementarAlumnoCurso  (IN id INT)
BEGIN	
	UPDATE curso
    SET cantalum = cantalum-1        
    where idcurso = id; 
...
DELIMITER //
CREATE PROCEDURE spAlumnosCurso(IN id INT)
BEGIN	
	Select * from Usuario
    where idrol = 3 and idcurso = id
    Order by dni desc;
...
DROP PROCEDURE IF EXISTS spActualizarCursoAlumno;
DELIMITER //
CREATE PROCEDURE spActualizarCursoAlumno(IN dniAlum INT, IN newIdCurso int)
BEGIN	
	
    DECLARE actualIdCurso int;      
    SELECT idcurso INTO actualIdCurso FROM USUARIO WHERE dni = dniAlum; 
    
    
    IF newIdCurso != actualIdCurso then
        begin
			UPDATE Usuario
			SET idcurso = IF(newIdcurso is not null, newIdcurso, idcurso)        
			where dni = dnialum;
		end;
		begin
			UPDATE Alumno
			SET idcurso = IF(newIdcurso is not null, newIdcurso, idcurso)        
			where dnialumno = dnialum;
		end;
		begin
			call spincrementarAlumnoCurso(newIdCurso);
            call spdecrementarAlumnoCurso(actualIdCurso);
		end;
    end if;    
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE spdecrementarProfeMateria(IN id INT)
BEGIN	
	UPDATE curso
    SET cantmate = cantmate-1,
...
DELIMITER //
CREATE PROCEDURE spGetAlumnosCurso (IN id INT)
BEGIN	
	SELECT * FROM USUARIO
    where idcurso = id AND idrol=3; 
END //
DELIMITER ;

DELIMITER //
CREATE PROCEDURE spGetIdCursoRegistro (IN id2 INT)
BEGIN	
	SELECT idcurso FROM cursomateriaprofesor
    where id = id2; 
END ;


CREATE DEFINER=`root`@`localhost` PROCEDURE `spLogin`(IN pi_dni INT, IN pi_clave VARCHAR(50), OUT idrol INT, OUT msj VARCHAR(250))
BEGIN
DECLARE v_idrol int;
DECLARE Cant int;  
    DECLARE v_activo int;
...
IF Cant = 1 THEN
BEGIN
/*SELECT idrol INTO v_idrol FROM USUARIO
WHERE dni = pi_dni AND clave = pi_clave;*/
                   
SELECT activo, idrol INTO v_activo, v_idrol FROM USUARIO
WHERE dni = pi_dni AND clave = pi_clave;                    
                       
                       IF  v_activo = FALSE THEN
SET msj = "Usuario inactivo";
                            SET idrol = 0;
ELSE
SET msj = "Usuario activo";
                             SET idrol = 1;
END IF;
END;
ELSE SET msj = "El Usuario no existe o la clave es incorrecta";
